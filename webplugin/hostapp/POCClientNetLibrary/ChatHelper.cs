...
namespace POCClientNetLibrary
{
    public static class ChatHelper
    {
        ...
        public class StateObject
        {
            ...
            //这个当buffer 取小时，有尾包，还是会报错
            public const int BUFFER_SIZE =64*1024 ;   //原来:128*1024 ,   2024.12.12 减小缓存，由于语音包一帧大小是18 个byte / 每20ms, 18*10*2= 400ms 延迟
            // Receive buffer.
        ...
        public static GroupSyncMessage  parseGroupSyncMsgData(byte[] msgBytes, int next, int lastLen, out int parselen)
        {
            GroupSyncMessage message = new GroupSyncMessage();
            int index = next;
            parselen = 0;
            try
            {
                ...
                byte messageLen = msgBytes[index + 2];  //为  (13 + groupNameLength)
        ...
        public static ManageDataPacket parseManageData( byte[] msgBytes, int next, int lastLen, out int parseLen )
        {
            ...
            //2024.12.27 加入sos会话消息
            string sos_sessionid;  //32位的guid去除-分隔符形式
            byte sos_state;  //1: 开始 2:结束
            ...
            switch (messageId)
            {
                ...
                case MyType.TYPE_SOS_SESSION:
                    ...
                    userId = IPAddress.NetworkToHostOrder(userId);
                    //
                    sos_sessionid = Encoding.GetEncoding("UTF-8").GetString(msgBytes, index + 13, 32);
                    sos_datetime = BitConverter.ToInt32(msgBytes, index + 13 + 32);
                    sos_datetime = IPAddress.NetworkToHostOrder(sos_datetime);

                    sos_state = msgBytes[index + 13 + 32+4];
                    //再设置data
                     
                    data.setSos_datetime(sos_datetime);
                    data.setGroupId(groupId);
                    data.setUserId(userId);                 
                    data.sos_sessionid=sos_sessionid;
                    data.sos_state = sos_state;
                ...
                case MyType.TYPE_SOS_LOCATION:
                    ...
                    sos_sessionid = Encoding.GetEncoding("UTF-8").GetString(msgBytes, index + 34, 32);

                    gps_type_len = BitConverter.ToInt16(msgBytes, index + 66);
                    gps_type_len = IPAddress.NetworkToHostOrder(gps_type_len);

                    if (gps_type_len > 0)
                    {
                        gps_type = Encoding.GetEncoding("UTF-8").GetString(msgBytes, index + 68, gps_type_len);
                    }

                    //取sosid, 这后面应该没有了
                    //sos_id = BitConverter.ToInt64(msgBytes, index + 68 + gps_type_len);
                ...
                case MyType.TYPE_GPS_COMMAND:
                    ...
                    break;
                default:
                    Debug.WriteLine("parseManageData 中也进入 default " );
                    return null;
    ...
    public class Data
    {
        ...
        public Data(byte[] data, int pos, int LastLen)
        {
            ...
            //MsgLen = data[next];   
            next += sizeof(byte);  //语音包，第三个字节是长度，因为采用arm编码，一帧20ms的编码长度不会超过256，所以用一个字节
            ...
            switch (Command)
            {
                ...
                case MyType.TYPE_TOPOC_END_EX:
                   ...
                   // Int32 i32_MsgLen = MsgLen < 0 ? 65535 + MsgLen : MsgLen;
                    Int32 i32_MsgLen = MsgLen;  //不会有负值

                    Debug.WriteLine("1. audio Data pos:" + pos + " cmd:" + Command + " len:" + i32_MsgLen + " las:" + LastLen);
                    ...
                    int bytenumPerpackage = (i32_MsgLen - 2) / 1;
                ...
                case MyType.MEDIA_EX_TOPLATFORM:
                    ...
                    Int32 i32_MsgLen_2 = MsgLen ;

                    Debug.WriteLine("1. MEDIA_EX_TOPLATFORM Data pos:" + pos + " cmd:" + Command + " len:" + i32_MsgLen_2 + " las:" + LastLen);
                    ...
                    int bytenumPerpackage_2 = (i32_MsgLen_2 - 2) / 1;
                    ...
                    Debug.WriteLine("MEDIA_EX_TOPLATFORM group_id:" + group_id);
                    Debug.WriteLine("MEDIA_EX_TOPLATFORM user_id:" + user_id);
        ...
        /// <summary>
        /// 调度台帐号登录用，这时还没有确定对讲群组，可传入 -1
        /// </summary>
        /// <param name="groupID"></param>
        /// <param name="userID"></param>
        /// <returns></returns>
        public byte[] LoginPlatformMessageEncode(int groupID, int userID)
        {
            var result = new List<byte>();

            short tmp = IPAddress.HostToNetworkOrder((short)MyType.TYPE_LOGIN_PLATFORM);
            result.AddRange(BitConverter.GetBytes(tmp));
        ...
        /// <param name="userId">被邀请的人ID</param>
        /// <param name="inviteId"> 邀请的同步用户ID</param>
        /// <returns></returns>
        ...
        public byte[] MediaEncode( byte[] outbuf )
        {
            ...
            short len = IPAddress.HostToNetworkOrder( (short) (2+outbuf.Length) );
    ...
    public static class MyType
    {
        ...
        public const short TYPE_LOGIN_PLATFORM = 81;  // 专用于调度台的登入
        ...
        public const short TYPE_SOS_SESSION = 111;
	 ...
	 */
        ...
        public String sos_sessionid { get; set; }
        public byte sos_state { get; set; }
    ...
    /// 监听别的组讲话的组ID和讲话人ID
    /// </summary>
    public class MonitorAudioMessage
    {
        public int groupId { get; set; }
        public int userId { get; set; }

        public byte[] outbuf { get; set; }
        public MonitorAudioMessage(int groupId, int userId, byte[] outbuf)
        {
            this.groupId = groupId;
            this.userId = userId;
            this.outbuf = outbuf;
