...
public interface IPTTSDK {
    ...
    /**
     * imei方式终端登录(适用于嵌入式和无屏终端登录)
     * @param imei
     * @param callback
     */
    void loginImei(String imei,LoginCallback callback);
     ...
     * @param callback
     */
    void updateUserLocation(Integer groupId, Double latitude, Double longitude, Integer batterylevel, Integer savelocday, UpdateUserLocationCallback callback);
    ...
    /**
     * 发送单呼信令
     * @param groupId  单呼所建组ID
     * @param fromUserId  发自用户ID
     * @param toUserId    目的用户ID
     * @param signalVal   信令值
     * @param callback
     */
    void sendSingleCallSignal(int groupId, int fromUserId, int toUserId, byte signalVal, CommonCallback callback);

    /**
     * 发送SOS会话消息
     * @param groupId  当前所在组ID
     * @param userId
     * @param sessionIdStr  SOS会话ID，32位GUID去除了分隔符 - 后的小写字符串，如 d22a4fa72e4042a79fe62a3a0e4be2a8
     * @param state   1 : 开始， 2：结束
     */
    void sendSOSSession(int groupId,int userId,String sessionIdStr,byte state,CommonCallback callback);
     ...
     * @param userId
     * @param sessionIdStr  SOS会话ID，32位GUID去除了分隔符 - 后的小写字符串，如 d22a4fa72e4042a79fe62a3a0e4be2a8
     * @param sosType   SOS类型, 报警级别,1表示SOS, 2 表示其它, 3表示是调度台发给POC
     * @param gpsType   定位类型: 如 baidu,gps,Google
     * @param longitude  经度
     * @param latitude   纬度
     */
    void sendSOSLocation(int groupId,int userId,String sessionIdStr,byte sosType,String gpsType, double longitude,double latitude,CommonCallback callback);
    ...
    /**
     * 设置 ptt键的状态，在gb28181对讲期间ptt不允许按
     * @param pttValidStatus true: 可以按ptt键,  false: 不允许按ptt键
     */
    void setPttValid(boolean pttValidStatus);
