...
public class VoipEvent implements ISkyEvent {
    ...
    public void createRoom(String room, int roomSize,String roomOtherUsers) {
        SocketManager.getInstance().createRoom(room, roomSize,roomOtherUsers);
    }
    ...
    public void shouldStartRing(boolean isComing) {
        Uri uri;
        if (isComing) {
            uri = Uri.parse("android.resource://" + MyPOCApplication.getInstance().getPackageName() + "/" + R.raw.income_ring);
        } else {
            uri = Uri.parse("android.resource://" + MyPOCApplication.getInstance().getPackageName() + "/" + R.raw.wr_ringback);
        }
