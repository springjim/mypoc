...
public class CallSingleActivity extends BaseActivity implements CallSession.CallSessionCallback {
    ...
    private String peerUserName;  //主叫对方名称或被邀请时对方的名称，即ptt_user表中的userName字段
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        peerUserName = intent.getStringExtra(EXTRA_USER_NAME);

        if (TextUtils.isEmpty(peerUserName))
            MyPOCApplication.getInstance().setWebrtcPeerUserName("webrtc");
        else
            MyPOCApplication.getInstance().setWebrtcPeerUserName(peerUserName);
    ...
    protected void onDestroy() {
        super.onDestroy();
        unregisterReceiver(headsetPlugReceiver);  //注销耳机监听
...
//        }
    ...
    private void init(String targetId, boolean outgoing, boolean audioOnly, boolean isReplace) {
        ...
        if (outgoing && !isReplace) {
            ...
            MyPOCApplication.getInstance().setRoomId(room);
            MyPOCApplication.getInstance().setOtherUserId(targetId);
    ...
    public void didCallEndWithReason(EnumType.CallEndReason reason) {
        MyPOCApplication.getInstance().setOtherUserId("0");
        //交给fragment去finish
//        finish();
