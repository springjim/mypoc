...
import com.mypoc.ptt.utils.StringUtil;
...
/**
 * websocket客户端，用于连接webrtc信令服务器
 */
public class MyWebSocket extends WebSocketClient {

    private final static String TAG = MyWebSocket.class.getSimpleName();
    ...
    public void onClose(int code, String reason, boolean remote) {
        ...
        if (connectFlag) {
           /* try {
            ...
            Log.i(TAG, "Connection closed, will reconnect in 10 seconds");
    ...
    public void onError(Exception ex) {
        ...
        // 发生错误也触发重连
        if (connectFlag) {
            Log.i(TAG, "Connection error, will reconnect in 10 seconds");
            this.iEvent.reConnect();
        } else {
    ...
    private void handleMessage(String message) {
        ...
        //添加进入会议模式提醒
        if (eventName.equals("__invitemeet")){
            handleInviteMeet(map);
            return;
        }
     ...
     * @param map
     */
    private void handleInviteMeet(Map map){
        //
        Log.i(TAG,"收到handleInviteMeet:"+ JSON.toJSONString(map));
        Map data = (Map) map.get("data");

        if (data != null) {
            String room = (String) data.get("room");
            String fromUserId = (String) data.get("fromUserId");
            this.iEvent.onInviteMeet(room, fromUserId);
        }
     ...
     * ------------------------------发送消息----------------------------------------
     */
    public void createRoom(String room, int roomSize, String myId,String roomOtherUsers) {
        ...
        childMap.put("otherUserIDs",roomOtherUsers);
