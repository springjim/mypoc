...
import com.mypoc.ptt.activity.keypad.ImeiActivity;
...
import com.mypoc.ptt.webrtc.socket.SocketManager;
...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        autoLogin();   // 这个不用了, 这个用于有触摸屏终端

        //无屏或按键类终端用 imei号登录，每次都调用下, 要等网络正常下，要延迟下
        // 最大重试5次，每次间隔2秒
        //getImeiWithRetry(8, 3000);

    }

    private void getImeiWithRetry(final int retryCount, final int delayMillis) {
            ...
            public void run() {
                try {
                    ...
                    if (imei != null && !imei.isEmpty()) {
                        Log.d(TAG, "Successfully got IMEI: " + imei);
                        imeiLogin(imei);
                    } else if (currentRetry < retryCount) {
                        currentRetry++;
                        Log.w(TAG, "Retry " + currentRetry + " to get IMEI");
                        handler.postDelayed(this, delayMillis);
                    } else {
                        Log.e(TAG, "Failed to get IMEI after " + retryCount + " attempts");
                    }
                } catch (Exception e) {
                    Log.e(TAG, "IMEI read error", e);
                }
            }
        }, delayMillis);
    }

    private void imeiLogin(String imei) {
        currImei= imei;
        // 检查网络状态
        if (NetworkUtil.is4GOrWifiAvailable(this)) {
            // 网络符合要求（4G/WiFi且可用）
            Log.d(TAG, "网络已就绪，开始登录流程");
            startLoginProcess();
        } else {
            // 网络不符合要求
            //showNetworkError();
            Log.d(TAG, "网络未准备好，开始监听...");
            // 注册网络变化监听（可选）
    ...
    private void startLoginProcess(){
        ...
        String imei= currImei;

        Log.e(TAG,"imei= "+imei);

        //如果取不到，则表示不支持app
        if (TextUtils.isEmpty(imei)){
            Toast.makeText(this,"不支持该终端，请确保是android6以下的系统",Toast.LENGTH_LONG).show();
            return;
        }
        //

        Log.e(TAG,"开始imei方式登录");
        // 初始化PTT管理器， 因为有服务IP配置
        //http://159.75.230.229:17003/ptt/
        String serverIp= "159.75.230.229";   //没地方输入，先写死
        //String serverIp= "192.168.105.13";     //广西客户内络服务IP，先写死
        ...
        PTTConfig config = new PTTConfig.Builder()
                ...
                .setPttDownBroadCastVal("qmstar.keyflag.ptt.down")       //ptt键按下的广播字符串,如 qmstar.keyflag.ptt.down, 这个因厂家而异，要咨询各厂家的技术人员
                .setPttUpBroadCastVal("qmstar.keyflag.ptt.up")           //ptt键抬起的广播字符串,如 qmstar.keyflag.ptt.up, 这个因厂家而异，要咨询各厂家的技术人员
        ...
        MyPOCApplication.getInstance().getPttSDK().loginImei( imei,
                new LoginCallback() {
                    @Override
                    public void onSuccess(String token, long expiresIn, String refreshToken, Integer userId) {
                        runOnUiThread(() -> {
                            ...
                            LoginPrefereces.setData_String(LoginActivity.this,
                                    LoginPrefereces.serverAddrKey,serverIp);

                            LoginPrefereces.setData_String(LoginActivity.this,
                                    LoginPrefereces.accountKey,userId+"");

                            //在退出时要写false
                            LoginPrefereces.setState(LoginActivity.this,
                                    LoginPrefereces.isLoggedInKey,true);

                            LoginPrefereces.setData_Long(LoginActivity.this, LoginPrefereces.lastLoginTimeKey,
                    ...
                    public void onFailure(String error) {
                        runOnUiThread(() -> {
                            ...
                            ////imei号没有绑定对讲帐号，调转到一个显示activity上
                            //imei号没有绑定对讲帐号，调转到一个显示activity上
                            Intent intent = new Intent(LoginActivity.this, ImeiActivity.class);
    ...
    private void autoLogin(){
        ...
        if (isLoggedIn && !TextUtils.isEmpty(account) && !TextUtils.isEmpty(password) && !TextUtils.isEmpty(serverIp)
            ...
            //先释放之前的资源
            if (MyPOCApplication.getInstance().getPttSDK()!=null){
                Log.e(TAG, "先释放之前的pttSDK");
                MyPOCApplication.getInstance().getPttSDK().release();
            }
            ...
            //webrtc的信令服务地址
            String signalServerUrl= "ws://" +serverIp+":17003/ptt/ws";
            ...
            MyPOCApplication.getInstance().getPttSDK().initialize(config, new InitializeCallback() {
                ...
                public void onSuccess() {
                    pttSDKStatus=true;
                    Log.d(TAG, "PTTSDK初始化成功");
                    String passwordDecrypt= AESHelper.decrypt(password,masterPasswordKey); //先解密
                            ...
                            new LoginCallback() {
                                ...
                                public void onSuccess(String token, long expiresIn, String refreshToken, Integer userId) {
                                    runOnUiThread(() -> {
                                        ...
                                        Log.i(TAG,"TOKEN="+token);
                                        //成功登录后写配置文件
                                        MyPOCApplication.getInstance().setToken(token);
                                        Intent mainIntent= new Intent(LoginActivity.this, MainActivity.class);
                                        mainIntent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
                                        startActivity(mainIntent);

                                        //连接webrtc信令服务
                                        connectSignal(signalServerUrl,account);
                                        finish();
                                    });
                                }
                                ...
                                new LoginCallback() {
                                    ...
                                    public void onSuccess(String token, long expiresIn, String refreshToken, Integer userId) {
                                        runOnUiThread(() -> {
                                            ...
                                            //连接webrtc信令服务
                                            connectSignal(signalServerUrl,etAccount.getText().toString().trim());
                                            finish();
                                        });
                                    }
    ...
    private void connectSignal(String signalServerUrl, String userId) {
        SocketManager.getInstance().connect(signalServerUrl,userId,0);
    }
