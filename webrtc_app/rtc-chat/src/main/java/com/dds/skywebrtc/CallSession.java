...
public class CallSession implements EngineCallback {
     ...
     * @param context
     * @param roomId
     * @param audioOnly  true 语音会议 false 视频会议
     * @param event
     */
    ...
    /**
     * 改进了，加了 roomOtherUsers, 用于创建会议模式时，指定其它与会人员的userId
     * @param room
     * @param roomSize
     * @param roomOtherUsers  指定其它与会人员的userId,以逗号隔开
     */
    public void createHome(String room, int roomSize, String roomOtherUsers) {
        executor.execute(() -> {
            if (mEvent != null) {
                mEvent.createRoom(room, roomSize,roomOtherUsers);
            }
        });
    }
